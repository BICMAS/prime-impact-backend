...
exports.ScormPackageModel = void 0;
...
class ScormPackageModel {
  ...
  static async uploadAndExtract(filePath, filename, uploadedBy, lessonId = null) {
    ...
    const packageSize = _fs.default.existsSync(filePath) ? _fs.default.statSync(filePath).size : 0;
    const {
      jobId,
      courseId
    } = await _ScormCloudService.ScormCloudService.submitCourseUpload(filePath, filename, lessonId);
    ...
    const pkg = await _db.prisma.scormPackage.create({
      data: {
        filename,
        storagePath: `scormcloud://${courseId}`,
        manifestJson: {},
        scormVersion: 'V2004',
        // sensible default; corrected once import finishes
        encrypted: false,
    ...
    if (_fs.default.existsSync(filePath)) _fs.default.unlinkSync(filePath);
  ...
  static async finalizeImportInBackground(jobId, courseId, filename, packageId) {
    try {
      const result = await _ScormCloudService.ScormCloudService.waitForImportJob(jobId, courseId, filename);
      const scormVersion = result.scormVersion?.includes('2004') || result.scormVersion?.includes('4th') ? 'V2004' : 'V1_2';
      await _db.prisma.scormPackage.update({
        where: {
          id: packageId
        },
        data: {
      ...
      try {
        await _db.prisma.scormPackage.delete({
          where: {
            id: packageId
          }
        });
  ...
  static async findById(id) {
    return _db.prisma.scormPackage.findUnique({
      where: {
        id
      },
      include: {
  ...
  static async getLaunchUrl(packageId, userId, userFullName, options = {}) {
    ...
    let scormAttempt = await _db.prisma.scormAttempt.findFirst({
      where: {
        userId,
        scormPackageId: packageId
      },
    ...
    const createFreshRegistrationAndLaunch = async () => {
      const result = await _ScormCloudService.ScormCloudService.createLaunchLink(scormCloudId, userId, userFullName || 'Learner');
    ...
    try {
      if (registrationId) {
        ...
        try {
          const launchRes = await client.post(`/registrations/${registrationId}/launchLink`, launchPayload);
        ...
        const courseAttempt = await getOrCreateCourseAttempt();
        scormAttempt = await _db.prisma.scormAttempt.create({
          data: {
            userId,
            scormPackageId: packageId,
...
//     }
// }
exports.ScormPackageModel = ScormPackageModel;