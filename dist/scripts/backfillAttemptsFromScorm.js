...
async function main() {
  ...
  const scormAttempts = await _db.prisma.scormAttempt.findMany({
    include: {
      attempt: {
        select: {
          id: true
  ...
  for (const scormAttempt of scormAttempts) {
    ...
    let targetAttempt = await _db.prisma.attempt.findFirst({
      where: {
    ...
    if (!targetAttempt && !scormAttempt.attemptId) {
      ...
      if (APPLY_MODE) {
        targetAttempt = await _db.prisma.attempt.create({
          data: {
            userId: scormAttempt.userId,
            scormPackageId: scormAttempt.scormPackageId,
            ...values
