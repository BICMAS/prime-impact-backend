...
exports.LearnerDashboardService = exports.HRCourseTrackingService = exports.DashboardService = void 0;
var _DashboardModel = require("../models/DashboardModel.js");
...
class DashboardService {
  static async getHRDashboard(orgId) {
    ...
    const [totalLearners, averageCompletion, overdueCourses, activeAssignments, topPerformers, completionByDepartment, courseStatus] = await Promise.all([_DashboardModel.DashboardModel.getTotalLearners(orgId), _DashboardModel.DashboardModel.getAverageCompletion(orgId), _DashboardModel.DashboardModel.getOverdueCourses(orgId), _DashboardModel.DashboardModel.getActiveAssignments(orgId), _DashboardModel.DashboardModel.getTopPerformers(orgId), _DashboardModel.DashboardModel.getCompletionByDepartment(orgId), _DashboardModel.DashboardModel.getCourseStatus(orgId)]);
    return {
      totalLearners,
  ...
  static async getSuperAdminDashboard() {
    ...
    const [activeLearners, completionRate, systemLoad, recentActivities, learningActivityGraph, recentActivity] = await Promise.all([_DashboardModel.DashboardModel.getActiveLearners(), _DashboardModel.DashboardModel.getCompletionRate(), _DashboardModel.DashboardModel.getSystemLoad(), _DashboardModel.DashboardModel.getRecentActivities(), _DashboardModel.DashboardModel.getLearningActivityGraph(), _DashboardModel.DashboardModel.getRecentActivity()]);
    return {
      activeLearners,
...
class LearnerDashboardService {
  static async getLearnerDashboard(user) {
    ...
    const [streak, points, learningHours, coursesDone, averageScore, learningPaths, learningActivity, currentCourse, unfinishedCourses] = await Promise.all([_DashboardModel.DashboardModel.getLearnerStreak(user.id), _DashboardModel.DashboardModel.getLearnerPoints(user.id), _DashboardModel.DashboardModel.getLearnerHours(user.id), _DashboardModel.DashboardModel.getCoursesDone(user.id), _DashboardModel.DashboardModel.getAverageScore(user.id), _DashboardModel.DashboardModel.getLearnerPaths(user.id), _DashboardModel.DashboardModel.getLearningActivity(user.id), _DashboardModel.DashboardModel.getCurrentCourse(user.id), _DashboardModel.DashboardModel.getUnfinishedCourses(user.id)]);
    return {
      streak,
      points,
...
exports.LearnerDashboardService = LearnerDashboardService;
class HRCourseTrackingService {
  ...
  static buildLearnerStats(attempts = [], unfinishedCourses = []) {
    ...
    const uniqueCompletedCourses = new Set(completedAttempts.map(a => a.courseId).filter(Boolean)).size;
    const avgCompletionRaw = attempts.length > 0 ? attempts.reduce((sum, a) => sum + (a.completionPercentage || 0), 0) / attempts.length : 0;
  ...
  static async getLearnerCourseTracking({
    orgId
  }, learnerId) {
    ...
    const attempts = await _AttemptModel.AttemptModel.findByUserId(learnerId);
    const currentCourse = await _DashboardModel.DashboardModel.getCurrentCourse(learnerId);
    const unfinishedCourses = await _DashboardModel.DashboardModel.getUnfinishedCourses(learnerId);
  ...
  static async getAllLearnersCourseTracking({
    orgId
  }, query = {}) {
    if (!orgId) throw new Error('HR must be in an organization');
    const {
      ...
      limit,
      offset
    } = HRCourseTrackingService.normalizeQueryFilters(query);
    const learners = await _UserModel.UserModel.findLearnersByOrgId(orgId);
    ...
    const tracking = await Promise.all(learners.map(async learner => {
      const attempts = await _AttemptModel.AttemptModel.findByUserId(learner.id);
      const currentCourse = await _DashboardModel.DashboardModel.getCurrentCourse(learner.id);
      const unfinishedCourses = await _DashboardModel.DashboardModel.getUnfinishedCourses(learner.id);
      const stats = HRCourseTrackingService.buildLearnerStats(attempts, unfinishedCourses);
      const progressStatus = HRCourseTrackingService.getProgressStatus({
        attempts,
        stats
      });
      return {
...
exports.HRCourseTrackingService = HRCourseTrackingService;